...
if isprop(cluster.AdditionalProperties, 'ProcsPerNode')
    procsPerNode = validatedPropValue(cluster.AdditionalProperties, 'ProcsPerNode', 'double', 0);
    if mod(procsPerNode, 1) ~= 0 || procsPerNode < 0
...
if ~isempty(commonSubmitArgs)
    additionalSubmitArgs = strtrim(sprintf('%s %s', additionalSubmitArgs, commonSubmitArgs)) %#ok<NOPRT>
end
...
% First we need to determine whether the scheduler is PBS Pro or TORQUE.
% From observation, the output of 'qstat --version' is different on PBS Pro
% and Torque.  On TORQUE it is of the form:
% 'Version: 6.1.2
%  Commit: 661e092552de43a785c15d39a3634a541d86898e'
% On PBS Pro it is of the form:
% 'pbs_version = 19.0.0'.
...
if ~contains(cmdOut, 'pbs_version')
    % The scheduler is TORQUE. The job array integration in this plugin script
    % does not support TORQUE so disable job arrays.
    return;
end

% If we get here, the scheduler is PBS Pro.
